...
const _onStartOrPlay = () => {
  ...
  inputVolume.setAttribute("disabled", "true");
  languageInput.setAttribute("disabled","true");
...
export const toogleAudioPlayer = (isShow) => {
  if (isShow) {
     ...
     * Increase rate
     */
    btnIncreaseRate.addEventListener("click", (event) => {
      event.preventDefault();

      let newVoiceRateValue = parseFloat(voiceRateElem.innerText) + 0.5;

      voiceRateElem.innerHTML = `${newVoiceRateValue}x`;

      if (newVoiceRateValue === 2) {
        btnIncreaseRate.style.cssText = disableBtnStyle;
      }

      if (newVoiceRateValue === 1) {
     ...
     * Decrease rate
     */
    btnDecreaseRate.addEventListener("click", (event) => {
      event.preventDefault();
      ...
      if (newVoiceRateValue === 0.5) {
        btnDecreaseRate.style.cssText = disableBtnStyle;
      }

      if (newVoiceRateValue === 1.5) {
